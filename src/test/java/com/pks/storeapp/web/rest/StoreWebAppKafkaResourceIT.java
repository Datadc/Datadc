...
import com.pks.storeapp.config.EmbeddedKafka;
import com.pks.storeapp.config.KafkaSseConsumer;
import com.pks.storeapp.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class StoreWebAppKafkaResourceIT {
    ...
    void producesMessages() throws InterruptedException {
        client.post().uri("/api/store-web-app-kafka/publish?message=value-produce").exchange().expectStatus().isNoContent();
    ...
    void consumesMessages() {
        ...
        String value = client
            .get()
            .uri("/api/store-web-app-kafka/consume")
