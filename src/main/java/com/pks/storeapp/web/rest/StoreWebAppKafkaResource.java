...
@RestController
@RequestMapping("/api/store-web-app-kafka")
public class StoreWebAppKafkaResource {

    private final Logger log = LoggerFactory.getLogger(StoreWebAppKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public StoreWebAppKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
